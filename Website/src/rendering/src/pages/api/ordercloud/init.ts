...
import {
  ANONYMOUS_USER_ID,
  PUBLIC_BUYER_NAME,
  SHARED_AUTH0_CLIENT_ID,
  SHARED_AUTH0_CLIENT_SECRET,
  SHARED_AUTH0_AUTHORIZATION_ENDPOINT,
  SHARED_AUTH0_TOKEN_ENDPOINT,
...
const handler: NextApiHandler<unknown> = async (request, response) => {
  ...
  try {
    ...
    const openIdConnect: OpenIdConnect = {
      ...
      ConnectClientID: SHARED_AUTH0_CLIENT_ID,
      ConnectClientSecret: SHARED_AUTH0_CLIENT_SECRET,
      AppStartUrl: `${hostedBuyerSiteUrl}{2}?oidcToken={0}&idpToken={1}`,
      AuthorizationEndpoint: SHARED_AUTH0_AUTHORIZATION_ENDPOINT,
      TokenEndpoint: SHARED_AUTH0_TOKEN_ENDPOINT,
      UrlEncoded: true,
