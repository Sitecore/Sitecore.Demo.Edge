import {
  ...
  getGuestProfileResponse as boxeverGetGuestProfileResponse,
} from './BoxeverService';
...
export async function getGuestProfileEmail(): Promise<string> {
  const email = (await boxeverGetGuestProfileResponse())?.data?.email;
  return email;
}
