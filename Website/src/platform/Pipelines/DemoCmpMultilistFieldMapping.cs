...
namespace Sitecore.Demo.Edge.Website.Pipelines
{
    public class DemoCmpMultilistFieldMapping : SaveFieldValues
    {
        ...
        public DemoCmpMultilistFieldMapping(ICmpConverterMapper mapper, BaseLog logger, CmpHelper cmpHelper, CmpSettings settings) : base(mapper, logger, cmpHelper, settings)
        {
            this._mapper = mapper;
                ...
                if (!string.IsNullOrEmpty(fieldName))
                {
                    if (!string.IsNullOrEmpty(str))
                    {
                        try
                        {
                            ...
                            {
                                Log.Info("tata =========================== "+fieldName + " " + str, this);
                                ...
                                {
                                    this.Logger.Error(BaseHelper.GetLogMessageText(DemoCmpMultilistFieldMapping._settings.LogMessageTitle, string.Format("Configuration of the field mapping '{0}' is incorrect. Required fields are not specified.", (object)obj.ID)), (object)this);
                                    flag = true;
                                    continue;
                                }
                                List<string> stringList = this._cmpHelper.TryMapRelationPropertyValues(args, cmpRelationName, str);
                                args.Item[fieldName] = stringList.Count != 0 ? string.Join(DemoCmpMultilistFieldMapping._settings.RelationFieldMappingSeparator, (IEnumerable<string>)stringList) : string.Empty;
                                continue;
                            }
                        ...
                        {
                            this.Logger.Error(BaseHelper.GetLogMessageText(DemoCmpMultilistFieldMapping._settings.LogMessageTitle,
                                $"An error occured during converting '{(object) str}' field to '{(object) fieldName}' field. Field mapping ID: '{(object) obj.ID}'."), ex, (object)this);
                            flag = true;
                            args.Exception = ex;
                            continue;
                        }
