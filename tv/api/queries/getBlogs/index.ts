...
export const getBlogs = async (preview: boolean): Promise<{ blogs: Blog[] }> => {
  try {
    const blogsQuery = `
    query {
      allM_Content_Blog {
    ...
    const results: BlogsQueryResult = (await fetchGraphQL(blogsQuery, preview)) as BlogsQueryResult;
    if (results) {
      return {
